                ...
                if re.match(self.csv_code_regexp, record_code) \
                    ...
                    if not accounts:
                        raise exceptions.Warning(
                            _("Error Account not found: %s!") % record_code)
                    ...
                    if partner and len(partner) > 1:
                        if accounts[0].user_type_id.type == 'receivable':
                            partner = partner.filtered(lambda x: x.customer
                                            and x.company_type == 'company')
                        else:
                            if accounts[0].user_type_id.type == 'payable':
